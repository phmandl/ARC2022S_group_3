...
filename=package_path+'/waypoints/log_results_curv.csv'
...
READ_ROUTE_bugfix=True    # bug in pp manchmal startpunkt verschoben
...
class pure_pursuit:
    ...
    def save_waypoint(self,x_path,y_path,):
        ...
        #                         data.twist.twist.linear.z]),2)
         ...
         file.write('%f, %f, %f, %f,%f, %f, %f, %f\n' % (self.position_x,
                                         self.position_y,
                                         self.speed,self.yaw,x_path,y_path,self.Dt_left,self.Dt_right))
        # x_m,y_m,w_tr_right_m,w_tr_left_m
        #file.write('%.4f, %.4f, %.4f, %.4f\n' % (self.position_x,
        #                                self.position_y,self.Dt_left,self.Dt_right))
    ...
    def lidar_callback(self, data):
        ...
        c_range = data.ranges[INDEX_RIGHT_SCAN_RAY]
        d_range = data.ranges[INDEX_RIGHT_SCAN_RAY - NR_RAYS_A_TO_B]
        ...
        self.beta = np.arctan2(d_range*np.cos(theta) - c_range, d_range*np.sin(theta))
          # Calc Dt
        self.Dt_left = b_range*np.cos(self.alpha)
        # Calc Dt
        self.Dt_right = c_range*np.cos(self.beta)
    ...
    def traj_callback(self, traj_msg):
        ...
        #    self.path_available=True
        # for iy in np.ndindex(self.path.shape):
        #     print('iy, ix',iy)
        #     print('pahtiy, 1:',self.path[iy, 1])
        #     print('paht:1, iy',self.path[1, iy])
    ...
    def odom_callback(self, odom_msg):
        ...
        self.speed = la.norm(np.array([odom_msg.twist.twist.linear.x, 
    ...
    def update_follow_trajectory(self):
        if self.path_available==False:
            ...
            self.n_log+=1
            if self.n_log>=3 and self.log_active==True:
                self.save_waypoint(x_w,y_w)
                self.n_log=0
            ...
            self.publish_waypoint_vis_msg(x_w, y_w)  
            #rospy.loginfo("%s returned the invalid value %s", x_w, y_w)
